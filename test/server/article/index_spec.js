...
describe('GET /articles', () => {
  ...
  it('should return a 200 repsonse', done => {
    api
      .get('/api/articles')
      .end((err, res) => {
  ...
  it('should return an array of valid article objects', done => {
    api
      .get('/api/articles')
      .end((err, res) => {
        res.body.forEach((article, articleIdx) => {
          article.translations.forEach((translation, translationIdx) => {
            expect(translation._id).to.exist;
            expect(translation.title).to.eq(articleData[articleIdx].translations[translationIdx].title);
            expect(translation.tagline).to.eq(articleData[articleIdx].translations[translationIdx].tagline);
            expect(translation.image).to.eq(articleData[articleIdx].translations[translationIdx].image);
            expect(translation.language).to.eq(articleData[articleIdx].translations[translationIdx].language);
            expect(translation.content).to.eq(articleData[articleIdx].translations[translationIdx].content);
          });
        });
        done();
      });
